...
<script lang="ts" setup>
  ...
  const getUploadBtnText = computed(() => {
    ...
    return isUploadingRef.value ? '上传中' : someError ? '重新上传失败' : '开始上传';
  });
  ...
  function beforeUpload(file: File) {
    ...
    if (maxSize && file.size / 1024 / 1024 >= maxSize) {
      createMessage.error(`文件大小不能超过 ${maxSize}M`);
      return false;
    }
  ...
  async function handleStartUpload() {
    ...
    if (fileListRef.value.length + props.previewFileList.length > maxNumber) {
      return createMessage.warning(`最多上传 ${maxNumber} 个文件`);
    }
    try {
  ...
  function handleOk() {
    ...
    if (isUploadingRef.value) {
      return createMessage.warning('没有可保存的文件');
    }
    ...
    if (fileList.length <= 0) {
      return createMessage.warning('保存失败');
    }
    fileListRef.value = [];
    closeModal();
